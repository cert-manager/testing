...
CACHE_HOST="storage.googleapis.com"
...
hash_toolchains () {
    ...
    local tool_versions
    tool_versions="PY:${python_version}"
...
make_bazel_rc () {
    ...
    local cache_url
    cache_url="https://${CACHE_HOST}/jetstack-bazel-cache/${cache_id}"

    echo "build --remote_http_cache=${cache_url} --google_credentials=${BAZEL_CACHE_CREDENTIALS:-}"
