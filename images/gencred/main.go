...
import (
	...
	corev1applyconfiguration "k8s.io/client-go/applyconfigurations/core/v1"
...
type clusterConfig struct {
	...
	// KubernetesSecret is the secret that the kubeconfig will be written to.
	KubernetesSecret *KubernetesSecretConfig `json:"kubernetesSecret,omitempty"`
	// Output is the local path for generated kubeconfig.
...
func writeConfig(c clusterConfig, clientset kubernetes.Interface, getKubeClient kubeClientGetter) error {
	var err error
	...
	if c.KubernetesSecret != nil {
		...
		ctx := context.Background()
		_, err = client.CoreV1().Secrets(c.KubernetesSecret.Namespace).Apply(
			ctx,
			corev1applyconfiguration.
				Secret(c.KubernetesSecret.Name, c.KubernetesSecret.Namespace).
				WithData(map[string][]byte{
					"config": kubeconfig,
				}),
			metav1.ApplyOptions{
				Force:        true,
				FieldManager: "gencred",
			},
		)
		if err != nil {
			return err
...
func main() {
	...
	getKubeClient := lazyGetLocalClusterClient()

	if o.refreshInterval == 0 {
		if err := runOnce(*c, o.filter, getKubeClient); err != nil {
			util.PrintErrAndExit(err)
		}
		return
	}
...
type kubeClientGetter func() (*kubernetes.Clientset, error)

func lazyGetLocalClusterClient() kubeClientGetter {
	return sync.OnceValues(func() (*kubernetes.Clientset, error) {
...
func runOnce(c config, filter filter, getKubeClient kubeClientGetter) error {
	...
	for _, cc := range c.Clusters {
		...
		if err := writeConfig(*cc, clientset, getKubeClient); err != nil {
			errs = append(errs, err)
			continue
		}
